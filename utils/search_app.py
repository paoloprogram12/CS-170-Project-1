...
class App(tk.Frame):
    ...
    def __init__(self, search_fn_map, master=None):
        ...
        self.master.title("Search Algorithms -- CS 170 UCR")
        ...
        '''
        self.costFn = lambda pos: 3 if pos in self.deep else 1 #0.5**pos[0]        
    ...
    def default_game(self, event=None):
        ...
        for i in range(0, 8):
           for j in range(0,30):
              self.deep.append((i, j))
        self.deep = tuple(self.deep)
        self.draw_grid()
        for coord in self.deep:
            self.draw_deep(coord) 
    ...
    def draw_path(self, movement):
        ...
        for a in movement:
            if a in self.ACTIONS:
                ...
                return #break
        #tk.messagebox.showinfo("", "A path was found with a total cost of {} ".format(cost))
        print("Total path cost: {}".format(cost))
